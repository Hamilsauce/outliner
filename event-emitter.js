...
export class EventEmitter extends EventTarget {
  ...
  removeListener(eventName, listener=EventTarget) {
    const listener2 = this.#eventRegistry
      .get(eventName)
  ...
  dispatch(eventName, detail = {}) {
    // this.self.dispatchEvent(new CustomEvent(eventName, { bubbles: true, detail }));

    this.#eventRegistry.get(eventName)
    .forEach((l, i) => {
      l.dispatchEvent(new CustomEvent(eventName, { bubbles: true, detail }));
    });
  }
}
